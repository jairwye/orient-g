...
export default function EquityEntryPage() {
  ...
  const points = useMemo(() => {
    // 一家公司一个点：按 city/province 查中心点经纬度（真实相对位置）
    if (!locRoot) return [];
    ...
    return entities
      .map((t) => {
        const provRaw = (t.geo?.province || "")?.toString().trim();
        const cityRaw = (t.geo?.city || "")?.toString().trim();
  ...
  function hexColor(weight: number, maxW: number) {
    ...
    <div className="p-6 md:p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          选择数据批次（snapshot）后进入工作台与图谱；标准化数据表可在此下载。资料包上传请在{" "}
      ...
      <div className="mb-6 grid gap-3 rounded-lg border border-zinc-800 bg-zinc-900/40 p-4 lg:grid-cols-2">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
          <Link href={links.analysis} className="rounded-md bg-zinc-100 px-3 py-2 text-xs font-medium text-zinc-900 hover:bg-white">
            打开全景工作台
          </Link>
          <Link href={links.targets} className="rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 hover:bg-zinc-900">
            打开目标公司列表
          </Link>
          <Link href={links.compare} className="rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 hover:bg-zinc-900">
            打开公司对比
          </Link>
        </div>
      </div>
      ...
      <div className="mb-6 rounded-lg border border-zinc-800 bg-zinc-950 p-4">
        <div className="mb-2 flex items-center justify-between gap-3">
          <div className="text-sm font-medium text-zinc-200">公司分布（中国）</div>
          <div className="text-xs text-zinc-500">
            {loading ? "加载中…" : error ? "加载失败" : `共 ${entities.length} 家`}
          </div>
        </div>
        {error && (
          <div className="mb-3 rounded-md border border-red-900/40 bg-red-950/40 p-3 text-xs text-red-200">
            读取公司地理信息失败：{error}
          </div>
        )}
        <div className="mb-2 flex flex-wrap items-center justify-end gap-2 text-xs text-zinc-500">
          <button
            type="button"
        ...
        <div
          className="relative h-[560px] w-full overflow-hidden rounded-md border border-zinc-800 bg-zinc-900/20"
          ...
          >
            ...
            >
              ...
              {hexbins.map((hb) => {
                ...
                const fill = hexColor(hb.weight, hexLegend.maxW);
                return (
                  <g
                    key={hb.key}
                    onClick={(e) => {
        ...
        {clusterOpen ? (
          <div className="mt-3 rounded-md border border-zinc-800 bg-zinc-950/60 p-3">
            <div className="mb-2 flex items-center justify-between gap-3">
              <div className="text-sm font-medium text-zinc-200">{clusterOpen.title}</div>
              <button
                type="button"
            ...
            <div className="max-h-56 overflow-auto rounded-md border border-zinc-800">
              {clusterOpen.items.map((it) => (
                <button
                  key={it.entity_id}
                  type="button"
                  className="flex w-full items-center justify-between gap-3 border-b border-zinc-800 bg-zinc-950 px-3 py-2 text-left text-sm text-zinc-200 hover:bg-zinc-900"
                  onClick={() =>
                    router.push(
        ...
        <div className="mt-2 text-xs text-zinc-500">
          说明：底图为真实中国行政区（省级）GeoJSON；密度层使用 Hexbin（六边形蜂窝）按公司数量/标的权重聚合。点击六边形可打开公司列表再进入全景页；支持滚轮缩放、按住拖拽平移。
        </div>
      </div>
