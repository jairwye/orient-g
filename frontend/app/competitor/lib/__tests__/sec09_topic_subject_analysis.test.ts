...
describe("buildTopicSubjectGroups sec-09-11", () => {
  const briefMd = [
    ...
    "",
    "**简要分析：** 可比公司C呈现三层产品结构——(1)全民流量型(游戏1/4)累计用户千万级、ARPU中低；(2)核心IP型(游戏2/3)ARPU极高(1,125-1,675元)、靠高付费用户驱动；(3)新游戏(游戏6/7)ARPU极高(2,019-3,742元)但用户体量极小。可比公司G游戏X全年仅新增10万用户，所有指标全线下滑，进入存量衰退。境外版本ARPU普遍低于境内，以铺量获客为主。",
  ].join("\n");
  ...
  it("简要分析按 zq / la 展示名拆分，不含境外段", () => {
    ...
    expect(la!.bullets[0]!.text).toMatch(/^可比公司G游戏X全年仅新增10万用户/);
...
describe("buildTopicSubjectGroups sec-09-9 运营产品", () => {
  it("发行地区：连续「和」连接的两公司合并一行", () => {
    const md = [
      "**发行地区——全球化分层。** 可比公司A和可比公司C产品标注\"全球\"发行的比例最高，可比公司D以\"国内\"为主，可比公司B以\"中国大陆\"为核心。可比公司F100%面向中国大陆(渠道A发行)。可比公司G《游戏X》无海外版本。",
    ].join("\n");
  ...
  it("产品数量卡片删去蓝本中的 > 分隔符", () => {
    const md = [
      "**产品数量与集中度。** 可比公司A37款>可比公司D17款>可比公司B15款>可比公司C12款>可比公司F10款>YYCQ7款>可比公司H6款>可比公司G4款。但产品数量不等于收入贡献——可比公司G4款中《游戏X》一款贡献98.2%收入。",
    ].join("\n");
