...
function splitByCompany(text: string): SubjectAnalysisGroup[] {
  ...
  for (const sent of sentences) {
    const companies = findCompaniesInText(sent);
  ...
  return order
    ...
    .map((company) => ({
      ...
      colKey: COMPANY_COLS.find((c) => colToLabel(c) === company) ?? "",
...
export function buildTopicSubjectGroups(markdown: string): TopicSubjectGroup[] {
  ...
  for (const chunk of raw.split(/\n\n+/)) {
    ...
    const subjects = splitByCompany(parsed.body);
