...
**Goal:** 财务后台（任意 `finance_path`）上传行业财报汇析 MD → 解析为 **竞品财报** Snapshot → 侧栏 `/competitor` 展示 10 屏 Recharts 页。
...
- [x] `backend/scripts/smoke_competitor_report.py`：admin upload + GET assert sections
