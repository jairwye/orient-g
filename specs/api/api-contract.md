...
| 字段 | 值 |
|------|-----|
| **status** | `baseline`（随实现演进，改接口须同步本文） |
| **canonical path** | `specs/api/api-contract.md` |

本文档约定各模块前后端接口形状，供实现与验收对照。原 `docs/api-contract.md` 已迁至此目录（规制类，与 `specs/ui/` 并列）。
...
> **说明**：供内网 `/equity` 及 `/analysis`、`/compare`、`/targets` 等页使用；**为临时增加能力，后续可能移除**。  
> **可见性（2026-05）**：侧栏入口与全部只读/导入 API **仅管理员**（`admin` 角色）；非管理员访问路由将重定向，接口返回 403。
