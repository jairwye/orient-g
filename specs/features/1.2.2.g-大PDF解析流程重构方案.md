...
|------|-----|
| **status** | `baseline` |
| **来源** | 原 `规划/1.2.2.g`（2026-05 迁入后原稿已删） |
| **实现** | `backend/services/bigpdf_tasks.py`、`backend/routers/knowledge.py`（bigpdf）、`frontend/app/utils/pdf-knowledge/` |
...
> **创建日期**：2026-05-12
> **关联版本**：1.2.2.g_
> **状态**：baseline（核心 API 与队列已落地；文内「插队」等未实现项见 §特别说明）
...
**真正的插队（暂停当前任务，优先处理新任务，完成后恢复旧任务）不可行。**

原因：
- Docling-serve 内部是单线程队列，不支持任务暂停/恢复
- 没有 API 可以"暂停当前解析，保存状态，稍后恢复"

**可以实现的替代方案：**
1. **队列优先级调整**：后端队列支持优先级设置，新任务可以设为高优先级，排在队列前面
2. **取消并重新开始**：用户可以选择取消当前排队任务，立即开始新任务
3. **多容器方案**：部署多个 docling 容器（资源消耗大，不推荐）

**本方案采用的策略：**

由于只能软取消（不终止 docling 进程），上传新文件时的选项逻辑：

| 选项 | 效果 | 说明 |
|------|------|------|
| **排队等待** | 新任务加入队列 | 当前任务完成后自动处理新任务 |
| **取消** | 不排队、不上传 | 仅关闭对话框，现有任务完全不受影响 |

**注意**：
- docling 是单线程，一次只能处理一个任务
- 新任务必须等当前任务完成后才能开始
- 取消按钮仅影响本次上传操作，不影响现有任务

### 关于"私人知识库"

**"私人知识库"是本仓库知识库系统的一个特定类别（`kb_kind="Private"`），自动创建的文件夹将归属此类别。**

实现方式：
- 知识库类别 `kb_kind="Private"` 仅允许文档所有者访问（`allow_owner: True`）
- 文件夹通过 `dynamic_private_collection_id(username)` 创建，格式为 `c_private_dyn_{username}`
- 文档通过 `kb_private_collection_owner` 表关联到所有者的私人集合
- 其他用户（包括管理员）默认不可见，除非用户主动共享到其他类别（如 DeptPublic、ProjectPublic 等）

**数据库关联：**
```sql
-- 私人集合所有者表
kb_private_collection_owner (
    tenant_id TEXT NOT NULL,
    private_collection_id TEXT NOT NULL,  -- 如：c_private_dyn_user001
    owner_username TEXT NOT NULL,
    PRIMARY KEY (tenant_id, private_collection_id)
)

-- 知识库类别定义
kb_kind = "Private"  -- 仅所有者可见
```
...
- 弹窗提示：
  ...
  **说明**：
  - **[排队等待]**：新文件加入队列，当前任务完成后自动处理
  - **[取消]**：不排队、不上传、不取消现有任务，仅关闭对话框（用户可稍后重新上传）
  - 现有任务不受影响，继续正常运行
...
def process_bigpdf_task(tenant_id, task_id, owner_username, is_cancelled=None):
    ...
    if res and not (is_cancelled and is_cancelled()):
        ...
        folder = kb_documents.create_folder(
            ...
            name=folder_name,
            parent_path="/",  # 私人知识库根目录
            kb_kind="Private",  # 明确指定为私人知识库类别
            auto_created=True,
        )
        
        # 将解析结果关联到私人知识库集合
        for doc_id in generated_docs:
...
│   │   └── ...
│   └── ...
├── 部门知识库 (kb_kind="DeptPublic")/
├── 项目知识库 (kb_kind="ProjectPublic")/
└── ...
```

**权限说明：**
- `kb_kind="Private"` 的文档仅所有者可访问（`allow_owner: True`）
- 文档通过 `kb_private_collection_owner` 表关联到所有者
- 集合 ID 格式：`c_private_dyn_{username}`
- 用户可通过分享功能将文档移动到其他类别（如部门、项目）
...
2. 用户上传时如果系统忙，提供两个选项：
   a. [排队等待]：新任务加入队列，当前任务不受影响
   b. [取消]：不排队、不上传，仅关闭对话框，现有任务不受影响
3. 队列按 FIFO 顺序处理
4. 队列对用户透明，显示排队位置和预计等待时间
5. 用户可通过进度卡片单独取消自己正在运行的任务（软取消）
6. 管理员/所有者可以强制终止当前任务（重启 docling 容器）
```
