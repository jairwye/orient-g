...
- **竞品财报（1.2.3.c）**：侧栏 `/competitor`（须 `view_business_dashboard`：管理层/财务部/管理员）；行业汇析 MD 在财务后台上传；**纵向对比**见 `/competitor/vertical`（`POST /api/competitor/admin/upload-vertical` 上传独立 MD）。详见 [specs/features/1.2.3.c-竞品财报可视化.md](specs/features/1.2.3.c-竞品财报可视化.md)。
