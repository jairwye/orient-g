...
def test_classify_report_opening_as_delta_not_根据():
    chunk = "根据检索到的华清飞扬2024年与2025年财务报告数据，以下是"
...
def test_normalize_glued_table_and_conclusion():
    ...
    raw = (
        "检索到的数据，以下是关于销售费用的对比分析报告。###华清2025-2024年销售费用对比分析报告"
        "####1.核心数据概览|项目 |2025年 |2024年 || :--- | :--- | :--- | :--- | :--- |"
        " | 销售费用 | 13,722,360.23 | 25,081,092.51 |"
    )
    out = normalize_reply_markdown(raw)
    assert "### 华清" in out or "###华清" in out
    assert "\n|" in out or "|\n|" in out
...
def test_strip_orchestration_preamble_before_report_title():
    ...
    raw = (
        ...
        "让我直接输出：---#华清飞扬2025年与2024年销售费用对比分析报告\n\n"
        "## 结论2025年销售费用合计13,722,360.23元。\n"
    )
    out = strip_hermes_orchestration_preamble(raw)
    assert out.startswith("#华清") or out.startswith("华清")
...
def test_strip_english_evidence_pack_preamble():
    ...
    raw = (
        ...
        "华清飞扬2025年营业收入对比分析报告结论：2025年营业收入为100,148,026.24元。"
    )
    out = strip_hermes_orchestration_preamble(raw)
    assert out.startswith("华清")
    assert "Evidence Pack" not in out[:40]
    fin = finalize_agent_reply(raw, user_query="华清2025年与2024年营业收入对比")
    assert "100,148,026.24" in fin
    assert not fin.lower().startswith("based on")
    q = "华清25、24两年销售费用明细的对比分析报告"
    fin = finalize_agent_reply(raw, user_query=q)
    assert fin.startswith("#") or fin.startswith("华清") or "结论" in fin[:40]
    assert "skill" not in fin[:200].lower() or "华清" in fin[:80]


def test_normalize_user_paste_glued_report():
    """浏览器实测常见：标题/表/小节挤一行、导语重复。"""
    raw = (
        "根据检索到的财务附注数据，以下是华清2025年与2024年销售费用的明细对比分析报告。\n"
        "华清2025-2024年销售费用明细对比分析报告\n"
        "1.费用总额与结构概览2025年，华清销售费用总额大幅缩减，较2024年下降了 45.29%。"
    ...
    out = finalize_agent_reply(raw, user_query="华清25、24两年销售费用明细的对比分析报告", tier2_native=True)
    assert "根据检索到的财务附注数据" not in out[:80] or "## 华清" in out
    assert "## 华清" in out or "#### 1." in out
    assert "| 销售费用合计 |" in out
...
def test_enforce_keeps_table_strips_yue_wan_in_prose():
    ...
    out = finalize_agent_reply(
        synth,
        user_query="出一份华清25、24两年研发费用明细的对比分析报告",
    )
...
def test_normalize_rd_glued_conclusion_pipe_table_and_shuoming():
    ...
    out = finalize_agent_reply(raw, user_query="华清25、24两年研发费用明细的对比分析报告")
...
def test_strip_inline_ud_id_and_document_brackets():
    ...
    raw = (
        "变动原因见 ud_0401544fb6f7425092db1d9f7a970917 与 [document 华清2025年报] "
